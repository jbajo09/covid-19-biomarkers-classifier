...
GSE156063 <- read.csv('GSE156063_summary.csv' ,header=TRUE, row.names = 1)
...
# SEXO: F (51%) - M(49%)
# AGE: 33-36 
GSE149273 <- read.csv('GSE149273_summary.csv' ,header=TRUE, row.names = 1)
...
GSE152075 <- read.csv('GSE152075_summary.csv' ,header=TRUE, row.names = 1)
...
GSE163151 <- read.csv('GSE163151_summary.csv' ,header=TRUE, row.names = 1)
...
GSE162835 <- read.csv('GSE162835_summary.csv' ,header=TRUE, row.names = 1)
...
train_labels_scale <- labels_scale[index]
test_labels_scale <- labels_scale[-index]
train_matrix_scale <- expression_matrix_norm_scale_fix_out[,index]
test_matrix_scale <- expression_matrix_norm_scale_fix_out[,-index]
...
gene_limma_scale2_matrix <- matrix(0,ncol = 5,nrow = length(names(gene_mrmr_scale2)))
rownames(gene_limma_scale2_matrix) <- names(gene_mrmr_scale2)
gene_limma_scale2_matrix <- gene_limma_scale2_matrix[order(rownames(gene_limma_scale2_matrix)),]
for (i in 1:length(rownames(DEGs_scale2$CV_Results$Fold_1$MulticlassLFC))){
  if (rownames(DEGs_scale2$CV_Results$Fold_1$MulticlassLFC)[i]%in%rownames(gene_limma_scale2_matrix)){ 
    gene_limma_scale2_matrix[which(rownames(gene_limma_scale2_matrix) == rownames(DEGs_scale2$CV_Results$Fold_1$MulticlassLFC)[i]),1]<- mean(DEGs_scale2$CV_Results$Fold_1$MulticlassLFC[i,])
  }
}
for (i in 1:length(rownames(DEGs_scale2$CV_Results$Fold_2$MulticlassLFC))){
  if (rownames(DEGs_scale2$CV_Results$Fold_2$MulticlassLFC)[i]%in%rownames(gene_limma_scale2_matrix)){ 
    gene_limma_scale2_matrix[which(rownames(gene_limma_scale2_matrix) == rownames(DEGs_scale2$CV_Results$Fold_2$MulticlassLFC)[i]),2]<- mean(DEGs_scale2$CV_Results$Fold_2$MulticlassLFC[i,])
  }
}
for (i in 1:length(rownames(DEGs_scale2$CV_Results$Fold_3$MulticlassLFC))){
  if (rownames(DEGs_scale2$CV_Results$Fold_3$MulticlassLFC)[i]%in%rownames(gene_limma_scale2_matrix)){ 
    gene_limma_scale2_matrix[which(rownames(gene_limma_scale2_matrix) == rownames(DEGs_scale2$CV_Results$Fold_3$MulticlassLFC)[i]),3]<- mean(DEGs_scale2$CV_Results$Fold_3$MulticlassLFC[i,])
  }
}
for (i in 1:length(rownames(DEGs_scale2$CV_Results$Fold_4$MulticlassLFC))){
  if (rownames(DEGs_scale2$CV_Results$Fold_4$MulticlassLFC)[i]%in%rownames(gene_limma_scale2_matrix)){ 
    gene_limma_scale2_matrix[which(rownames(gene_limma_scale2_matrix) == rownames(DEGs_scale2$CV_Results$Fold_4$MulticlassLFC)[i]),4]<- mean(DEGs_scale2$CV_Results$Fold_4$MulticlassLFC[i,])
  }
}
for (i in 1:length(rownames(DEGs_scale2$CV_Results$Fold_5$MulticlassLFC))){
  if (rownames(DEGs_scale2$CV_Results$Fold_5$MulticlassLFC)[i]%in%rownames(gene_limma_scale2_matrix)){ 
    gene_limma_scale2_matrix[which(rownames(gene_limma_scale2_matrix) == rownames(DEGs_scale2$CV_Results$Fold_5$MulticlassLFC)[i]),5]<- mean(DEGs_scale2$CV_Results$Fold_5$MulticlassLFC[i,])
  }
}
gene_limma_scale2 <- rownames(gene_limma_scale2_matrix)[order(apply(gene_limma_scale2_matrix, 1, mean))]
...
#Validation plot
plot(knn_train_mrmr_scale2$accuracyInfo$meanAccuracy[10:15], type = 'l', col= 'black', ylab='Metric Performance', xlab='Genes', lwd=2, ylim = c(0.8,1), panel.first = grid(col='gray45'))
lines(knn_train_mrmr_scale2$sensitivityInfo$meanSensitivity[10:15], col='blue', lwd=2, lty=2)
lines(knn_train_mrmr_scale2$specificityInfo$meanSpecificity[10:15], col='#FF8B00', lwd=2, lty=4)
lines(knn_train_mrmr_scale2$F1Info$meanF1[10:15], col='red', lwd=2, lty=4)
...
#Validation plot
plot(svm_train_mrmr_scale2$accuracyInfo$meanAccuracy, type = 'l', col= 'black', ylab='Metric Performance', xlab='Genes', lwd=2, ylim = c(0,1), panel.first = grid(col='gray45'))
lines(svm_train_mrmr_scale2$sensitivityInfo$meanSensitivity, col='blue', lwd=2, lty=2)
lines(svm_train_mrmr_scale2$specificityInfo$meanSpecificity, col='#FF8B00', lwd=2, lty=4)
lines(svm_train_mrmr_scale2$F1Info$meanF1, col='red', lwd=2, lty=4)
legend(x=14.305 ,y =0.8345, c('Accuracy', 'Sensitivity','Specificity','F1-Score'), lty = c(1,2,4,5), col = c('black','blue','#FF8B00','red'))
...
# tsne
par(oma = c(5, 1, 0, 1))
tsne(expression_matrix_norm_scale_fix_out[which(rownames(expression_matrix_norm_scale_fix_out)%in%names(gene_mrmr_scale2)[1:15]),],labels=as.factor(labels_scale),controlscale=TRUE, scale=3, colvec = c('red','blue','green','black'),axistextsize = 0, seed = 5)
